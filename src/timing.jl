...
function add_timing_constraints!(model::Model, data::Data)
    ...
    @unpack m_local_arrival, m_input_capacity, d_setup_to_recaiming = data
    ...
    # When a pile finishes building on a position, quality assersion must happen until the next day
    @constraint(model, [ps in s_positions, t in s_periods[1:end-1]],
        D_QUALITY_ASSERTION[ps,t] + D_QUALITY_ASSERTION[ps,t+1] >= d_setup_to_recaiming[ps] * B_POSITION_END_BUILDING[ps,t]
    )
